    ...
    def create(original, short=None, validate=False):
        ...
        if validate:
            if len(original) > MAXIMUM_LENGTH_ORIGINAL:
                raise InvalidAPIUsage(URLMap.ERROR_LONG_ORIGINAL)

            if short:
                if len(short) > MAXIMUM_LENGTH_SHORT:
                    raise InvalidAPIUsage(URLMap.ERROR_INVALID_SHORT_URL)
                if not re.match(SHORT_PATTERN, short):
                    raise InvalidAPIUsage(URLMap.ERROR_INVALID_SHORT_URL)
